...
def main(config_path, path_to_store_hf_dataset, convokit_corpus_dir=None):
    with open(config_path, 'r') as fp:
        config = yaml.safe_load(fp)
    convokit_corpus_dir = str(Path.home()) if convokit_corpus_dir is None else convokit_corpus_dir

    corpus = Corpus(filename=download(config['convokit']['id'], data_dir=convokit_corpus_dir))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_to_store_hf_dataset', type=str, help='path to store huggingface dataset',
                        default=os.getcwd())
    parser.add_argument('--convokit_corpus_dir', type=str, help='path to downloaded convokit corpus',
